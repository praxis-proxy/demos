#!/usr/bin/env bash
# Record the streaming multi-turn demo with asciinema inside a tmux session.
#
# Layout:
#   ┌─────────────────────┬────────────────────┐
...
SESSION="praxis-streaming-multi-turn-demo"
PRAXIS_BIN="${PRAXIS_BIN:-$(command -v praxis-ai 2>/dev/null || echo "")}"
CONFIG="$SCRIPT_DIR/streaming-multi-turn.yaml"
...
tmux split-window -h -t "$SESSION" \
    "printf '\033[1;33m── Praxis Logs ──\033[0m\n'; \
     ...
     | grep --line-buffered -E 'classified|mode|stateless|stateful|rehydrat|responses_proxy|rebuild|store|persist|stream_events|accumul|terminal|route matched|upstream selected|listening|ready'"
...
asciinema rec \
    --overwrite \
    --title "Praxis — Streaming Multi-Turn with previous_response_id" \
    --idle-time-limit 3 \
