#!/usr/bin/env python3
"""Interactive LLM agent in front of the Praxis policy gateway.
...
class GatewayClient:
    """Calls tools through the Praxis policy gateway. The agent sends
    ...
    def __init__(self, gateway_url: str, client_token: str, user_token: str):
        ...
        # agent.session_id, and the engine keys session taint by H(subject:session_id)
...
def format_tool_response(status: int, data: dict[str, Any]) -> str:
    ...
    if status >= 400:
        ...
        # The gateway puts the violation code in X-Policy-Violation but
...
def run_chat(
    ...
    console.print(
        Panel(
            ...
            title="[bold]Praxis Policy Engine HR Demo[/bold]",
...
def main() -> int:
    p = argparse.ArgumentParser(description="LLM agent in front of the Praxis policy gateway")
    p.add_argument(
        "--persona",
        default="alice",
    ...
    p.add_argument(
        ...
        help=f"Policy gateway endpoint (default: {DEFAULT_GATEWAY})",
    )
    p.add_argument(
        "--keycloak",
