...
```bash
cd ..
./restart.sh                          # builds praxis + brings up Keycloak/MCP
```
...
You're now talking to the LLM, which is in front of the policy gateway.
...
**In the gateway's stderr (the gateway process):**

```json
{"ts":"2026-…","plugin":"audit-log","source":"policy-demo-gateway",
