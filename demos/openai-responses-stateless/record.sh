#!/usr/bin/env bash
# Record the demo with asciinema inside a tmux session.
#
# Layout:
#   ┌─────────────────────┬────────────────────┐
...
CONFIG="$SCRIPT_DIR/passthrough.yaml"
...
tmux split-window -h -t "$SESSION" \
    "printf '\033[1;33m── Praxis Logs ──\033[0m\n'; \
     ...
     | grep --line-buffered -E 'classified|mode|stateless|stateful|route matched|upstream selected|listening|ready'"
...
asciinema rec \
    --overwrite \
    --title "Praxis — OpenAI Responses API Stateless Passthrough" \
    --idle-time-limit 3 \
